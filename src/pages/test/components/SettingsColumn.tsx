...
function SettingsColumn(): ReactElement {
  ...
  const transitionTransform = `transform -translate-x-full transition-transform ${
    isDisplayingSettingsModal && '-translate-x-0'
  }`;
  ...
  const handleSettingsTabClick = () => {
    //This method is used to handle if someone click the settings button
  ...
  return (
    <React.Fragment>
      ...
      <svg
        ...
        className={`feather xl:ml-8 feather-settings hover:text-gray-400 text-white cursor-pointer active:text-gray-700 ${
          isDisplayingSettingsModal == true ? 'absolute' : 'absolute'
        }`}
      >
