...
const AggregateRow = ({ data }: { data: any }) => {
  ...
  const trainingSetting = useStoreState(
  ...
  useEffect(() => {
    ...
    /* eslint-disable */
    const canCHMTierBeUnlocked =
      parseInt(Math.max.apply(Math, Object.values(maxWPM))?.toFixed()) * 5 >
      200;
    /* eslint-enable */
    ...
    if (
      canCHMTierBeUnlocked &&
      JSON.parse(localStorage.getItem('FirstTimeEnteringCHMTier') == null)
    ) {
