...
export default class PianoKeyBoard extends React.Component<any, any> {
  ...
  getRecordingEndTime = () => {
    ...
    return Math.max(
      ...this.state.recording.events.map(
        (event: { time: any; duration: any }) => event.time + event.duration,
      ),
    );
  };
      ...
      this.state.recording.scheduledEvents.push(
        setTimeout(() => {
          const currentEvents = this.state.recording.events.filter(
            (event: { time: number; duration: any }) => {
  ...
  onClickStop = () => {
    this.state.recording.scheduledEvents.forEach(
      (scheduledEvent: number | undefined) => {
