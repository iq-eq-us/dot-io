...
} from '../../pages/manager/controls/mainControls';
import {
...
function deleteChordInManager(store, payload) {
  ...
  store.downloadedChords.chords.splice(
    store.downloadedChords.chords.findIndex(
      (v) =>
        v.currentChord === currentChord && v.currentPhrase === currentPhrase,
    ),
    1,
  );
...
function saveEditedChordInManager(store, payload) {
  ...
  const ChordStruct = store.downloadedChords.chords.find(
    (c: ChordStructure) =>
      c.currentChord === currentChord && c.currentPhrase === currentPhrase,
  ) as ChordStructure;
  ...
  store.downloadedChords = {
    chords: store.downloadedChords.chords.map((e: ChordStructure) =>
      e.currentChord === currentChord && e.currentPhrase === currentPhrase
        ? ChordStruct
        : e,
    ),
  };
